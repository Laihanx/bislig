<?php
// manage_transportation.php - CRUD for transportation
session_start();
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM transportation WHERE id = ?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "i", $id);
    if (mysqli_stmt_execute($stmt)) {
        $message = 'Transportation option deleted successfully!';
    } else {
        $error = 'Error deleting transportation option.';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $sql = "UPDATE transportation SET name=?, description=?, operating_hours=?, rating=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssdi", $name, $description, $operating_hours, $rating, $id);
    } else {
        $sql = "INSERT INTO transportation (name, description, operating_hours, rating) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssd", $name, $description, $operating_hours, $rating);
    }

    if (mysqli_stmt_execute($stmt)) {
        $message = $id ? 'Transportation option updated successfully!' : 'Transportation option added successfully!';
    } else {
        $error = 'Error saving transportation option.';
    }
    mysqli_stmt_close($stmt);
}

$result = mysqli_query($conn, "SELECT * FROM transportation ORDER BY id DESC");

$edit_item = null;
if (isset($_GET['edit'])) {
    $edit_id = intval($_GET['edit']);
    $edit_stmt = mysqli_prepare($conn, "SELECT * FROM transportation WHERE id = ?");
...
<head>
    ...
    <title>Manage Transportation - Admin System</title>
    ...
    <style>
        ...
        .form-group input[type="text"], .form-group input[type="number"], .form-group textarea { width: 100%; padding: 0.7rem 0.9rem; font-family: var(--font-family); font-size: 0.9rem; color: var(--neutral-dark); border: 1px solid rgba(0, 0, 0, 0.15); border-radius: var(--border-radius); background-color: white; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.06); transition: all 0.3s ease; box-sizing: border-box; }
...
<body>
    ...
    <main class="main-content">
        <section class="content-section">
            ...
            <?php endif; ?>
            <!-- Add/Edit Modal -->
            <div class="confirmation-modal" id="transportationModal">
                <div class="confirmation-modal-content" style="max-width:600px;width:100%;min-height:unset;border-radius:12px;padding:25px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.3);border:3px solid #1d4ed8;text-align:left;display:flex;flex-direction:column;justify-content:center;">
                    <style>
                        #transportationModal .form-group { margin-bottom: 0.85rem; }
                        #transportationModal input, #transportationModal textarea {
                        ...
                        #transportationModal textarea {
                            min-height: 80px;
                        }
                        #transportationModal .form-actions {
                            margin-top: 1.1rem;
                        }
                    </style>
                    <h3 id="modalFormTitle"><i class="fas fa-plus-circle"></i> Add New Transportation Option</h3>
                    <form id="transportationForm" method="post">
                        <input type="hidden" name="id" id="modal_id" />
                        <div class="form-group">
                            <label for="modal_name"><i class="fas fa-bus"></i> Name</label>
                            <input type="text" id="modal_name" name="name" required placeholder="Enter transportation name" />
                        </div>
                        <div class="form-group">
                            <label for="modal_description"><i class="fas fa-align-left"></i> Description</label>
                            <textarea id="modal_description" name="description" rows="4" required placeholder="Enter description"></textarea>
                        </div>
                        <div class="form-group">
                            <label for="modal_operating_hours"><i class="fas fa-clock"></i> Operating Hours</label>
                            <input type="text" id="modal_operating_hours" name="operating_hours" placeholder="e.g. 6:00 AM - 9:00 PM" />
                        </div>
                        <div class="form-group">
                            <label for="modal_rating"><i class="fas fa-star"></i> Rating (0-5)</label>
                            <input type="number" step="0.1" min="0" max="5" id="modal_rating" name="rating" value="0" />
                        </div>
                        <div class="form-actions">
                            <button type="submit" class="btn btn-primary" id="modalSubmitBtn">
                                <i class="fas fa-save"></i> Add Transportation
                            </button>
            ...
            <div class="card">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                    <h2><i class="fas fa-list"></i> All Transportation Options</h2>
                                    ...
                                    <tr>
                                        ...
                                        <td class="actions">
                                            <button class="btn btn-warning btn-edit"
                                                ...
                                                data-operating_hours="<?= htmlspecialchars($row['operating_hours']) ?>"
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center">
                                            <i class="fas fa-inbox"></i> No transportation options found. Add your first option above!
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <div class="confirmation-modal" id="confirmationModal">
        <div class="confirmation-modal-content">
            ...
            <p id="modalMessage">Are you sure you want to delete this transportation option?</p>
    ...
    <script>
        ...
        deleteBtns.forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                ...
                const transName = btn.getAttribute('data-name');
                modalTitle.textContent = 'Delete Transportation Option';
                modalMessage.textContent = `Are you sure you want to delete "${transName}"? This action cannot be undone.`;
                modalConfirmBtn.href = `?delete=${transId}`;
        ...
        document.addEventListener('keydown', function(event) {
            if (event.key === 'Escape') {
                modal.classList.remove('active');
                transportationModal.classList.remove('active');
            }
        });
        ...
        const modal_operating_hours = document.getElementById('modal_operating_hours');
        const modal_rating = document.getElementById('modal_rating');
        const modalSubmitBtn = document.getElementById('modalSubmitBtn');

        // Open Add Modal
        openAddModalBtn.addEventListener('click', function() {
            modalFormTitle.innerHTML = '<i class="fas fa-plus-circle"></i> Add New Transportation Option';
            modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Add Transportation';
            modal_id.value = '';
            modal_name.value = '';
            modal_description.value = '';
            modal_operating_hours.value = '';
            modal_rating.value = '0';
            transportationModal.classList.add('active');
        });
        ...
        document.querySelectorAll('.btn-edit').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                modalFormTitle.innerHTML = '<i class="fas fa-edit"></i> Edit Transportation Option';
                modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Update Transportation';
                ...
                modal_operating_hours.value = btn.getAttribute('data-operating_hours');
