<?php
// manage_restaurants.php - CRUD for restaurants
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $sql = "UPDATE restaurants SET name=?, description=?, location=?, phone=?, email=?, badge=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssssi", $name, $description, $location, $phone, $email, $badge, $id);
        ...
        $sql = "INSERT INTO restaurants (name, description, location, phone, email, badge) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssss", $name, $description, $location, $phone, $email, $badge);
        $success = mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        if ($success) {
            $_SESSION['success_message'] = 'Restaurant added successfully!';
            header('Location: manage_restaurants.php');
            exit;
        } else {
...
if (isset($_GET['edit'])) {
    ...
    $edit_stmt = mysqli_prepare($conn, "SELECT * FROM restaurants WHERE id = ?");
...
<head>
    ...
    <title>Manage Restaurants - Admin System</title>
    ...
    <style>
        ...
        /* Form inside modal - compact spacing */
        ...
        .confirmation-modal .form-group textarea {
...
<body>
    ...
    <main class="main-content">
        <section class="content-section">
            ...
            <div style="margin-bottom: 1.5rem; display:none;"></div>

            <!-- Add/Edit Modal -->
            <div class="confirmation-modal" id="restaurantModal">
                <div class="confirmation-modal-content" style="max-width:600px;min-height:unset;text-align:left;display:flex;flex-direction:column;justify-content:center;">
                    <h3 id="modalFormTitle"><i class="fas fa-plus-circle"></i> Add New Restaurant</h3>
                    <form id="restaurantForm" method="post">
                        <input type="hidden" name="id" id="modal_id" />
                        <div class="form-group">
                            <label for="modal_name"><i class="fas fa-utensils"></i> Restaurant Name</label>
                            <input type="text" id="modal_name" name="name" required placeholder="Enter restaurant name" />
                        </div>
                        <div class="form-group">
                            <label for="modal_description"><i class="fas fa-align-left"></i> Description</label>
                            <textarea id="modal_description" name="description" rows="4" required placeholder="Enter restaurant description"></textarea>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="modal_email"><i class="fas fa-envelope"></i> Email</label>
                            <input type="email" id="modal_email" name="email" placeholder="Enter email address" />
                        </div>
                        <div class="form-group">
            ...
            <div class="card">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                    <h2><i class="fas fa-list"></i> All Restaurants</h2>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="7" class="text-center">
                                            <i class="fas fa-inbox"></i> No restaurants found. Add your first restaurant above!
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <div class="confirmation-modal" id="confirmationModal">
        <div class="confirmation-modal-content">
            ...
            <p id="modalMessage">Are you sure you want to delete this restaurant?</p>
    ...
    <script>
        ...
        deleteBtns.forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                ...
                const restName = btn.getAttribute('data-name');
                modalTitle.textContent = 'Delete Restaurant';
                modalMessage.textContent = `Are you sure you want to delete "${restName}"? This action cannot be undone.`;
                modalConfirmBtn.href = `?delete=${restId}`;
        ...
        openAddModalBtn.addEventListener('click', function() {
            modalFormTitle.innerHTML = '<i class="fas fa-plus-circle"></i> Add New Restaurant';
            modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Add Restaurant';
            modal_id.value = '';
            modal_name.value = '';
        ...
        document.querySelectorAll('.btn-edit').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                modalFormTitle.innerHTML = '<i class="fas fa-edit"></i> Edit Restaurant';
                modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Update Restaurant';
                ...
                modal_email.value = btn.getAttribute('data-email');
