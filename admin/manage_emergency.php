<?php
// manage_emergency.php - CRUD for emergency contacts
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $sql = "UPDATE emergency_contacts SET name=?, phone=?, description=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssi", $name, $phone, $description, $id);
    } else {
        $sql = "INSERT INTO emergency_contacts (name, phone, description) VALUES (?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sss", $name, $phone, $description);
    }

    if (mysqli_stmt_execute($stmt)) {
        $message = $id ? 'Contact updated successfully!' : 'Contact added successfully!';
    } else {
...
$result = mysqli_query($conn, "SELECT * FROM emergency_contacts ORDER BY id DESC");

$edit_item = null;
if (isset($_GET['edit'])) {
    $edit_id = intval($_GET['edit']);
    $edit_stmt = mysqli_prepare($conn, "SELECT * FROM emergency_contacts WHERE id = ?");
...
<head>
    ...
    <title>Manage Emergency Contacts - Admin System</title>
...
<body>
    ...
    <main class="main-content">
        <section class="content-section">
            ...
            <?php endif; ?>
            <!-- Add/Edit Modal -->
            <div class="confirmation-modal" id="emergencyModal">
                <div class="confirmation-modal-content" style="max-width:600px;width:100%;min-height:unset;border-radius:12px;padding:25px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.3);border:3px solid #1d4ed8;text-align:left;display:flex;flex-direction:column;justify-content:center;">
                    <style>
                        #emergencyModal .form-group { margin-bottom: 0.85rem; }
                        #emergencyModal input, #emergencyModal textarea {
                        ...
                        #emergencyModal .form-actions {
                            margin-top: 1.1rem;
                        }
                    </style>
                    <h3 id="modalFormTitle"><i class="fas fa-plus-circle"></i> Add New Contact</h3>
                    <form id="emergencyForm" method="post">
                        <input type="hidden" name="id" id="modal_id" />
                        <div class="form-group">
                            <label for="modal_name"><i class="fas fa-user"></i> Name</label>
                            <input type="text" id="modal_name" name="name" required placeholder="Enter contact name" />
                        </div>
                        <div class="form-group">
                            <label for="modal_phone"><i class="fas fa-phone"></i> Phone</label>
                            <input type="text" id="modal_phone" name="phone" required placeholder="Enter phone number" />
                        </div>
                        <div class="form-group">
                            <label for="modal_description"><i class="fas fa-align-left"></i> Description</label>
                            <textarea id="modal_description" name="description" rows="4" placeholder="Enter description"></textarea>
                        </div>
            ...
            <div class="card">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                    <h2><i class="fas fa-list"></i> All Emergency Contacts</h2>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center">
                                            <i class="fas fa-inbox"></i> No emergency contacts found. Add your first contact above!
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <div class="confirmation-modal" id="confirmationModal">
        <div class="confirmation-modal-content">
            ...
            <p id="modalMessage">Are you sure you want to delete this contact?</p>
    ...
    <script>
        ...
        deleteBtns.forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                ...
                modalMessage.textContent = `Are you sure you want to delete "${contactName}"? This action cannot be undone.`;
                modalConfirmBtn.href = `?delete=${contactId}`;
        ...
        openAddModalBtn.addEventListener('click', function() {
            modalFormTitle.innerHTML = '<i class="fas fa-plus-circle"></i> Add New Contact';
            modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Add Contact';
            modal_id.value = '';
            modal_name.value = '';
            modal_phone.value = '';
        ...
        document.querySelectorAll('.btn-edit').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                modalFormTitle.innerHTML = '<i class="fas fa-edit"></i> Edit Contact';
                modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Update Contact';
