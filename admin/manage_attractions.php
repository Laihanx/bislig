<?php
// manage_attractions.php - CRUD for attractions
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $sql = "UPDATE attractions SET name=?, description=?, location=?, rating=?, badge=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssdsi", $name, $description, $location, $rating, $badge, $id);
    } else {
        $sql = "INSERT INTO attractions (name, description, location, rating, badge) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssds", $name, $description, $location, $rating, $badge);
    }

    if (mysqli_stmt_execute($stmt)) {
        $message = $id ? 'Attraction updated successfully!' : 'Attraction added successfully!';
    } else {
        $error = 'Error saving attraction.';
    }
    mysqli_stmt_close($stmt);
}

$result = mysqli_query($conn, "SELECT * FROM attractions ORDER BY id DESC");

$edit_item = null;
if (isset($_GET['edit'])) {
    $edit_id = intval($_GET['edit']);
    $edit_stmt = mysqli_prepare($conn, "SELECT * FROM attractions WHERE id = ?");
...
<head>
    ...
    <title>Manage Attractions - Admin System</title>
...
<body>
    ...
    <main class="main-content">
        <section class="content-section">
            ...
            <?php endif; ?>
            <!-- Add/Edit Modal -->
            <div class="confirmation-modal" id="attractionModal">
                <div class="confirmation-modal-content" style="max-width:600px;width:100%;min-height:unset;border-radius:12px;padding:25px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.3);border:3px solid #1d4ed8;text-align:left;display:flex;flex-direction:column;justify-content:center;">
                    <style>
                        #attractionModal .form-group { margin-bottom: 0.85rem; }
                        #attractionModal input, #attractionModal textarea {
                        ...
                        #attractionModal textarea {
                            min-height: 80px;
                        }
                        #attractionModal .form-actions {
                            margin-top: 1.1rem;
                        }
                    </style>
                    <h3 id="modalFormTitle"><i class="fas fa-plus-circle"></i> Add New Attraction</h3>
                    <form id="attractionForm" method="post">
                        <input type="hidden" name="id" id="modal_id" />
                        <div class="form-group">
                            <label for="modal_name"><i class="fas fa-star"></i> Attraction Name</label>
                            <input type="text" id="modal_name" name="name" required placeholder="Enter attraction name" />
                        </div>
                        <div class="form-group">
                            <label for="modal_description"><i class="fas fa-align-left"></i> Description</label>
                            <textarea id="modal_description" name="description" rows="4" required placeholder="Enter attraction description"></textarea>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" step="0.1" min="0" max="5" id="modal_rating" name="rating" value="0" placeholder="Enter rating" />
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-actions">
                            <button type="submit" class="btn btn-primary" id="modalSubmitBtn">
                                <i class="fas fa-save"></i> Add Attraction
                            </button>
            ...
            <div class="card">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                    <h2><i class="fas fa-list"></i> All Attractions</h2>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" class="text-center">
                                            <i class="fas fa-inbox"></i> No attractions found. Add your first attraction above!
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <div class="confirmation-modal" id="confirmationModal">
        <div class="confirmation-modal-content">
            ...
            <p id="modalMessage">Are you sure you want to delete this attraction?</p>
    ...
    <script>
        ...
        deleteBtns.forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                ...
                const attName = btn.getAttribute('data-name');
                modalTitle.textContent = 'Delete Attraction';
                modalMessage.textContent = `Are you sure you want to delete "${attName}"? This action cannot be undone.`;
                modalConfirmBtn.href = `?delete=${attId}`;
        ...
        openAddModalBtn.addEventListener('click', function() {
            modalFormTitle.innerHTML = '<i class="fas fa-plus-circle"></i> Add New Attraction';
            modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Add Attraction';
            modal_id.value = '';
            modal_name.value = '';
        ...
        document.querySelectorAll('.btn-edit').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                modalFormTitle.innerHTML = '<i class="fas fa-edit"></i> Edit Attraction';
                modalSubmitBtn.innerHTML = '<i class="fas fa-save"></i> Update Attraction';
